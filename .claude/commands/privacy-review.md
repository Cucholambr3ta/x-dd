...
## 0. Pre-flight
- Crea esqueleto atomico `docs/privacy/` si no existe (INDEX + 1 doc por categoria PII).
- Requiere `DOMAIN.md` y `SPEC.md` para identificar entidades con PII.

### Salida atomica — docs/privacy/ (ADR-0050)

```
docs/privacy/
  INDEX.md / INDEX.json     (inventario PII: campo, categoria, sensible, base legal, retencion)
  identificacion.md / .json (PII de identificacion: nombre, DNI, pasaporte)
  contacto.md / .json       (email, telefono, direccion)
  financiero.md / .json     (tarjetas, cuentas, transacciones)
  salud.md / .json          (datos de salud — GDPR Art. 9, si aplica)
  biometrico.md / .json     (huellas, facial — GDPR Art. 9, si aplica)
  subprocesadores.md        (cross-cutting — allowed-multi)
  derechos-titular.md       (cross-cutting — allowed-multi)
PRIVACY.md                  → agregado generado (compat)
```

Cada `docs/privacy/<categoria>.md` documenta esa categoria de PII: campos, base legal,
retencion, transferencias. Tras escribir: `xdd-doc-sync.py sync-folder docs/privacy`.
PRIVACY.md se genera como agregado. Verificar:
```bash
python3 scripts/xdd-discipline-check.py folder --kind privacy --path docs/privacy
```
