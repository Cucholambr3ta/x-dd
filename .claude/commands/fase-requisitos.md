...
## 3.5 SALIDA ATOMICA — features/ y domain/ (ADR-0050)

FEATURES.md y DOMAIN.md se materializan de forma atomica: 1 doc = 1 concepto.

### Features → `docs/features/`

```
docs/features/
  INDEX.md / INDEX.json    (tabla: FEAT-NNN, nombre, prioridad, dominio, estado)
  <feature-slug>.md / .json (FEAT-NNN: nombre, prioridad MoSCoW, Gherkin completo, REQ refs)
docs/FEATURES.md           → agregado generado (compat, banner "GENERADO")
.xdd/briefing/FEATURES.md  → copia gate UNCHANGED (agregado valido para check_fdd + HMAC)
```

Por cada feature, `docs/features/<slug>.md` contiene: FEAT-NNN, nombre en lenguaje de
negocio, prioridad MoSCoW/RICE, criterios de aceptacion en Gherkin (happy + error + borde),
referencias a REQ-NNN. Tras escribir: `xdd-doc-sync.py sync-folder docs/features`.
...
docs/domain/
  INDEX.md / INDEX.json
  UBIQUITOUS_LANGUAGE.md   (glosario — exento de atomicidad)
  <aggregate-slug>.md / .json (bounded context: entidades, invariantes, domain events)
docs/DOMAIN.md             → agregado generado (compat)
.xdd/spec/DOMAIN.md        → copia gate UNCHANGED (valido para check_ddd + HMAC)
```

Por cada aggregate, `docs/domain/<slug>.md` con su bounded context. El glosario
ubicuo va en `UBIQUITOUS_LANGUAGE.md`. Tras escribir: `xdd-doc-sync.py sync-folder docs/domain`.

**Importante:** las copias en `.xdd/briefing/` y `.xdd/spec/` quedan como agregados
validos UNCHANGED — el gate las firma y `check_fdd`/`check_ddd` las validan. Solo el
rich tree (`docs/`) se atomiza. Verificar:

```bash
python3 scripts/xdd-discipline-check.py folder --kind features --path docs/features
python3 scripts/xdd-discipline-check.py folder --kind domain --path docs/domain
```
